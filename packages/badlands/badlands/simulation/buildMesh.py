...
def construct_mesh(input, filename, verbose=False):
    ...
    * define the partitioning when parallelisation is enable.
    ...
    FVmesh.edge_length = np.zeros((totPts, 20), dtype=float, order="F")
    FVmesh.vor_edges = np.zeros((totPts, 20), dtype=float, order="F")
    FVmesh.control_volumes = np.zeros(totPts, dtype=float)
